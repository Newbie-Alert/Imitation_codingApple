...
MongoClient.connect(process.env.DB_URL, function (err, client) {
  ...
  app.get('/islogin/confirm', function (req, res) {
  ...
  //COURSE PAGE
  //============
  app.get('/data/course', function (req, res) {
  ...
  // DETAIL PAGE
  //=============
  app.get('/data/detail/:id', function (req, res) {
  ...
  app.get('/data/board/:id1/:id2', function (req, res) {
  ...
  app.get('/data/board/content/:id1/:id2', function (req, res) {
  ...
  app.post('/reply', function (req, res) {
    db.collection('comment').insertOne({ title: req.body.title, comment: req.body.comment, time: req.body.time, re_comment: [] })
    console.log('추가')
  })

  app.get('/comment/:id', function (req, res) {
    db.collection('comment').find({ title: req.params.id }).toArray(function (err, result) {
      res.json(result);
    })

    app.post('/rereply', function (req, res) {
      db.collection('comment').updateOne({ _id: ObjectId(req.body.target) }, {
        $push: { re_comment: { title: req.body.title, comment: req.body.comment, time: req.body.time, re_comment: [] } }, function(err, result) {
          console.log('대댓 추가')
        }
      })
    })
  ...
  app.get('/length/cart', function (req, res) {
  ...
  app.get('/', function (req, res) {
    res.sendFile(path.join(__dirname, 'codingabbble/build/index.html'))
  })
